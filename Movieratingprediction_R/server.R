  ...
  function(input,output,session){
    observe({
      ...
      isolate({
        ...
        #####
        print("start ann")
        movie_data <- dbGetQuery(jdbcConnection, 
                                 paste0('select * from MOVIE_METADATA1 where rown=5045'))
        print(movie_data)
        movie_data$IMDB_SCORE <- as.numeric(movie_data$IMDB_SCORE)
        str(movie_data)
        print(movie_data$IMDB_SCORE)
        print(movie_data$TITLE_YEAR)
        ...
        scaled_data <- data.frame(lapply(movie_num, function(x) scale(x, center = FALSE, scale = max(x, na.rm = TRUE))))
        print("adi4")
        print(movie_num)
        scaled_data$IMDB_SCORE = movie_num$IMDB_SCORE / 10
        print("adi5")
        ...
        print("adi6")
        scaled_data$ROWN=movie_num$ROWN
        ...
        inpt1=scaled_data[scaled_data$ROWN == "5045",]
        load("ann_model")
        predd <- neuralnet::compute(ann_model, inpt1[,-1])
        inpt1$predictin=predd$net.result
        print("adiann2")
        ...
        dbRemoveTable(jdbcConnection, "MYTABLE3")
        ...
        dbWriteTable(jdbcConnection, "MyTable3",  inpt1 , row.names=FALSE, append=TRUE)
        ######
        ...
        oupt1 = dbGetQuery(jdbcConnection, paste0("select IMDB_SCORE from MYTABLE2" ))
        print(oupt1)
        
        output$result = renderPrint({print(oupt1)})
